...
import { Shield, ArrowRight, Activity, Radio, Cpu } from 'lucide-react';
...
const TICKER_DATA = [
  'SENSOR-WEH-NH8 // Vibration +2.4mm/s² ↑ ELEVATED',
  'SYNC: 12 road segments — 180 sensor feeds NOMINAL',
  'CONTRACTOR: Omega Infrastructure — BLACKLIST FLAG ACTIVE',
  'STRESS-ZONE: S.V. Road // Sub-base delamination detected',
  'BUDGET: LBS Marg — ₹14.2Cr allocated / 87% utilization',
  'COMPLAINT SPIKE: JVLR-SH1 — 14 reports in 72h window',
  'REPAIR-INTEGRITY: WEH-NH8 / Patch 7 — bond loss detected',
  'DRAINAGE: SCLR — Saturation 91% // Critical threshold exceeded',
  'AUTHORITY: NHAI-ROM — Budget variance +₹2.1Cr flagged',
  'LIVE FEED: 12 active monitoring zones // 4 critical alerts',
];
    ...
    <div className="fixed inset-0 z-[2000] bg-[#050507] flex flex-col overflow-hidden select-none">
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-2.5 border-b border-white/[0.04] bg-black/30 backdrop-blur-sm shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="mono-label text-[9px]">ROADWATCH-INTELLIGENCE // BUILD 2.0-OPERATIONAL</span>
        </div>
        <div className="flex items-center gap-6">
          <span className="mono-label hidden sm:block">
            <span className="text-emerald-400/60">●</span> 12 SEGMENTS ONLINE
          </span>
          <span className="mono-label hidden md:block">
            <span className="text-rose-400/60">●</span> 4 CRITICAL ALERTS
          </span>
      ...
      <div className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-0 min-h-0 overflow-hidden">
        ...
        {/* Left Column */}
        <div className="lg:col-span-7 flex flex-col justify-center px-8 lg:px-14 py-10 space-y-8 relative z-10 overflow-y-auto">
          ...
          <div className="animate-fade-in-up stagger-1 opacity-0">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm border border-cyan-500/20 bg-cyan-950/15 text-cyan-500/80">
              <span className="mono-label text-[9px] tracking-[0.2em]">NATIONAL INNOVATION INITIATIVE // CLEARANCE: OPERATIONAL</span>
            </div>
          </div>

          {/* Logo + Title */}
          <div className="space-y-5 animate-fade-in-up stagger-2 opacity-0">
            <div className="flex items-center gap-4">
              <div className="relative flex items-center justify-center w-12 h-12 rounded-xl border border-cyan-500/20 bg-cyan-950/20">
                <Shield className="w-6 h-6 text-cyan-400" />
                <div className="absolute inset-0 rounded-xl animate-glow-breathe" />
              </div>
              <div>
                <div className="mono-label text-[8px] mb-0.5">MINISTRY OF URBAN DEVELOPMENT</div>
                <h1 className="text-3xl sm:text-4xl font-black tracking-[0.06em] text-slate-100 uppercase">
                  ROAD<span className="text-cyan-400">WATCH</span>
                </h1>
              </div>
            </div>
            <p className="text-sm text-[#6a6a7a] leading-relaxed max-w-lg font-medium">
              Infrastructure intelligence platform for civil road accountability. 
              Real-time sensor monitoring, contractor integrity scoring, and 
              geospatial audit visualization across municipal networks.
            </p>
          </div>

          {/* Capability cards */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 animate-fade-in-up stagger-3 opacity-0">
            {[
              {
                icon: Activity,
                color: 'text-cyan-400',
                ...
                label: 'GIS Audit',
                desc: 'Spatial road segment monitoring with ward boundary overlays'
              },
              {
                icon: Radio,
                ...
                desc: '180 virtual sensor feeds across 12 road segments'
              },
              {
                icon: Cpu,
                ...
                label: 'Intelligence',
                desc: 'Deterministic scoring, contractor audits, and diagnostics'
              },
            ].map(({ icon: Icon, color, bg, label, desc }) => (
              <div key={label} className={`p-4 rounded-lg border ${bg} space-y-2 glass-card`}>
                <div className={`flex items-center gap-2 ${color}`}>
                  <Icon className="w-3.5 h-3.5 shrink-0" />
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-200">{label}</span>
                </div>
                <p className="text-[9px] text-[#55555f] leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>

          {/* Metrics row */}
          <div className="flex items-center gap-6 animate-fade-in-up stagger-4 opacity-0">
            {[
              ...
              <div key={label} className="text-center">
                <div className="mono-readout text-base">{val}</div>
                <div className="mono-label text-[8px] mt-0.5 whitespace-nowrap">{label}</div>
              </div>
            ))}
          </div>

          {/* CTA Buttons */}
          <div className="flex flex-col sm:flex-row gap-3 animate-fade-in-up stagger-5 opacity-0">
            <button
              onClick={onEnterDirect}
              className="group flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-slate-950 text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-cyan-500/20 active:scale-95"
            >
              Launch Platform
            ...
            <button
              onClick={onStartTour}
              className="flex items-center justify-center gap-2 px-6 py-3.5 rounded-lg border border-white/[0.08] bg-white/[0.03] text-slate-300 hover:text-slate-100 hover:border-white/[0.14] text-xs font-black uppercase tracking-widest transition-all active:scale-95"
            >
              Interactive Tour
            </button>
            {onStartDemo && (
          ...
          <div className="mono-label text-[8px] animate-fade-in-up stagger-6 opacity-0">
            MINISTRY OF URBAN DEVELOPMENT &amp; INFRASTRUCTURE OVERSIGHT © 2026 // CLASSIFICATION: PUBLIC
          </div>
        </div>
        ...
        <div className="lg:col-span-5 relative bg-black/20 border-l border-white/[0.03] overflow-hidden min-h-[320px]">
          {/* Corner tags */}
          <div className="absolute top-3 left-3 z-10 mono-label text-[8px] opacity-50">SYS:3D-TWIN // RENDER-LIVE</div>
          <div className="absolute top-3 right-3 z-10 mono-label text-[8px] opacity-50">CAM:INSPECTION-01</div>
          <div className="absolute bottom-3 left-3 z-10 mono-label text-[8px] opacity-50">STATUS: NOMINAL</div>
      ...
      <footer className="relative z-10 flex items-center gap-4 px-6 py-2 border-t border-white/[0.04] bg-black/30 backdrop-blur-sm shrink-0 overflow-hidden">
        <span className="mono-label text-[9px] text-cyan-500/60 shrink-0 tracking-[0.2em]">LIVE TELEMETRY</span>
